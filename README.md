...
### 7. 사람 추종 · 가이드 (비전 추종 서브시스템) — 🔧 설계 단계 · 미구현
관리자 **추종**(로봇이 특정 사람을 따라감) · 방문자 **가이드**(로봇이 앞서 안내하며 뒤따라오는지 확인).
**학습 차이가 아니라 시나리오(참조 데이터 출처) 차이** — 두 경우 동일 파이프라인. 관리자=사전등록 임베딩(영속 DB), 가이드=즉석 캡처 임베딩(임시 저장 후 삭제).

> ⚠️ 이 repo(arte_libi_fleet)에는 아직 비전/추종 코드 없음(grep 확인) — 아래는 설계. **LIDAR는 장애물 회피 전용**(타깃 거리 측정엔 미사용).

**① 검출** — `YOLO11n` 사람 bbox
- 테스트가 **실물 사람이 아니라 3D 프린팅 사람 모형** → 사전학습이 모형을 person 으로 못 잡으면 **경량 fine-tune** 필요.
- 순서: ①사전학습 그대로 + conf 임계 낮춰 검출되는지 **확인** → 안 되면 ②모형 프레임 **오토라벨**(사전학습/Grounding-DINO)→검수→**class 0(person)으로** COCO 가중치에서 전이학습(실물 사람 검출 유지).
- ⚠️ **학습 라벨링**(모형을 위치·각도 무관 전부 라벨) ≠ **런타임 타깃선택**(검출된 사람 중 중앙 것 선택) — 분리.

**② 식별**
- `ByteTrack` — 프레임 간 ID 유지 (단기 구분)
- `ReID + HSV 색히스토그램(상체 영역)` — 외형으로 대상 특정 (장기 구분). 상체만 써 배경 오염 회피 · 뒷모습에 강함.

**③ 위치/거리**
- bbox center-x → **방위각** (중앙 이탈 = 좌우 각도)
- bbox **높이** → 거리 (넓이보다 자세 변화에 강건)
- LIDAR = 장애물 회피 전용 (타깃 ranging 아님)

**④ 대상유지** (보이는 대상 안 놓치기)
- Track Coasting — detection 끊겨도 Kalman 예측으로 버팀 (`track_buffer`) + **타임아웃**
- Online Gallery Update — 뒷모습 임베딩 축적(각도 변화 대응) · ⚠️ **드리프트/오염 가드 필요**(안정 track + 기존 gallery와 임계 이상 유사 시에만 갱신)

**⑤ 대상복구** (놓친 대상 되찾기)
- LKD(마지막 관측 방향)로 base 회전 → 재획득
- 실패 시 **정지 대기 → 타임아웃 종료** (탐색 Behavior Tree 는 이 단순 회복이 부족할 때 후순위)

**⑥ 제어**
- PID (거리 → 전/후진 · 방위각 → Visual Servoing: base 회전으로 대상을 화면 중앙 유지)
- 후진 = **후방 클리어런스(LIDAR 장애물 레이어) 확인 시에만** · setpoint 거리에 **데드밴드**(전/후진 떨림 방지)

**구현 순서(권장)**: ①검출 + ③방위각·거리 → **P제어 폐루프(한 사람 실제 추종)** 먼저 검증 → ②식별 → ④유지 → ⑤복구.
...
- **사람 추종·가이드 비전 서브시스템**(§7) = 설계만 됨. 먼저 사전학습 YOLO11n 이 3D 프린팅 모형을 person 으로 잡는지 확인 → 필요 시 경량 fine-tune → **P제어 폐루프(한 사람 추종)** 우선 검증.

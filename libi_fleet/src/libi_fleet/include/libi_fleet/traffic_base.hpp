...
public:
  ...
  // priority: 로봇의 현재 task 우선순위(높을수록 우선). 교착 시 최저 우선순위가 양보.
  //  GRANT   : 목표 노드+진행 엣지 예약 성공.
  ...
  virtual MoveDecision request_move(const std::string & robot, int from_node, int to_node,
                                    int priority) = 0;
  // robot 이 node 점유 해제(그 노드에서 출발하던 엣지 예약도 함께 해제). 취소/정지용.
  virtual void release(const std::string & robot, int node) = 0;
  // 출발 순간: 떠나는 노드만 해제(진행 엣지는 유지 → 이동 중 정면충돌 방지).
  virtual void release_node(const std::string & robot, int node) = 0;
  // 도착 순간: 지나온 엣지만 해제.
  virtual void release_edge(const std::string & robot, int from, int to) = 0;
